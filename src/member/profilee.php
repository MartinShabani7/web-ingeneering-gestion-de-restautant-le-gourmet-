                ...
                if (!in_array($_FILES['avatar']['type'], $allowedTypes)) {
                    ...
                    try {
                        ...
                        if (move_uploaded_file($_FILES['avatar']['tmp_name'], $uploadPath)) {
                            ...
                            // DEBUG: Afficher l'URL de test
                            error_log("DEBUG: Nouvel avatar uploadé: " . $uploadPath);
                            error_log("DEBUG: URL accessible: http://localhost:8080/uploads/avatars/" . $fileName);
                            ...
                            if ($user['avatar']) {
                                ...
                                if ($oldPath && file_exists($oldPath)) {
                                    unlink($oldPath);
                                    error_log("DEBUG: Ancien avatar supprimé: " . $oldPath);
                                }
                            }
...
error_log("DEBUG - Avatar en base: " . ($user['avatar'] ?? 'NULL'));
error_log("DEBUG - URL générée: " . getAvatarUrl($user['avatar'] ?? ''));
?>
<!DOCTYPE html>
<html lang="fr">
<head>
